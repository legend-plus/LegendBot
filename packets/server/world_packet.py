...
class WorldPacket(Packet):
    ...
    def decode(cls, data: bytes):
        ...
        world_height: int = struct.unpack(">L", data[8:12])[0]
        world_width: int = struct.unpack(">L", data[12:16])[0]
        world_size: int = struct.unpack(">L", data[16:20])[0]
        world: bytes = data[20:20+world_size]
        world_word_size: int = struct.unpack(">L", data[20+world_size:24+world_size])[0]

        bump_height: int = struct.unpack(">L", data[24 + world_size:28 + world_size])[0]
        bump_width: int = struct.unpack(">L", data[28 + world_size:32 + world_size])[0]
        bump_size: int = struct.unpack(">L", data[32+world_size:36+world_size])[0]
        bump_world: bytes = data[36+world_size:36+world_size+bump_size]
        bump_word_size: int = struct.unpack(">L", data[36+world_size+bump_size:40+world_size+bump_size])[0]
